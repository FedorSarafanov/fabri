...
\begin{scope}[xscale=-1,xshift=-0.1cm]
...
\xdef\p{20.55}
\xdef\N{1.7}
\addplot[name path=psinus1,sin,domain={(-\Rad+\Shift)*0.95:(\Rad-\Shift)*0.95},samples=400] {\wo*sqrt(1+(\coeff*x/\wo^2)^2)*sin((abs(x)<\leff?\omegga*\N:\omegga)*x/(1/\p*(abs(x)-\p))/2+\phhi)*(abs(x)<\leff?\scr:1)} node[pos=.8, above]{};

\addplot[name path=psinus2,sin,domain={(-\Rad+\Shift)*0.95:(\Rad-\Shift)*0.95},samples=400] {-\wo*sqrt(1+(\coeff*x/\wo^2)^2)*sin((abs(x)<\leff?\omegga*\N:\omegga)*x/(1/\p*(abs(x)-\p))/2+\phhi)*(abs(x)<\leff?\scr:1)} node[pos=.8, above]{};
